...
import { SalaryExemptions, SalaryBreakdown, HomeLoanInterest } from './types';
import { InputField } from './InputField';
import { Shield, ChevronDown, ChevronUp, Home, Building2 } from 'lucide-react';
...
export const ExemptionsSection: React.FC<ExemptionsSectionProps> = ({ exemptions, salary, onChange }) => {
  ...
  const updateField = (field: keyof SalaryExemptions, value: number | boolean | HomeLoanInterest) => {
    onChange({ ...exemptions, [field]: value });
  };

  const updateHomeLoanInterest = (field: keyof HomeLoanInterest, value: number | boolean) => {
    onChange({
      ...exemptions,
      homeLoanInterest: { ...exemptions.homeLoanInterest, [field]: value }
    });
  };
  ...
  return (
    <div className="card-elevated p-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
      ...
      <div className="space-y-4">
        ...
        <Collapsible open={homeLoanOpen} onOpenChange={setHomeLoanOpen}>
          <CollapsibleTrigger className="flex items-center justify-between w-full p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
            <div className="flex items-center gap-2">
              <Building2 className="w-4 h-4 text-blue-600" />
              <span className="font-medium text-sm">Section 24(b) - Home Loan Interest</span>
            </div>
            {homeLoanOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </CollapsibleTrigger>
          <CollapsibleContent className="pt-4">
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputField
                  ...
                  value={exemptions.homeLoanInterest.interestPaid}
                  onChange={(v) => updateHomeLoanInterest('interestPaid', v)}
                  tooltip="Total interest paid on home loan during the financial year"
                />
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium">Property Type</Label>
                  <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                    <span className={`text-sm ${!exemptions.homeLoanInterest.isSelfOccupied ? 'font-medium' : 'text-muted-foreground'}`}>
                      Let Out
                    </span>
                    <Switch
                      checked={exemptions.homeLoanInterest.isSelfOccupied}
                      onCheckedChange={(checked) => updateHomeLoanInterest('isSelfOccupied', checked)}
                    />
                    <span className={`text-sm ${exemptions.homeLoanInterest.isSelfOccupied ? 'font-medium' : 'text-muted-foreground'}`}>
                      Self Occupied
                    </span>
